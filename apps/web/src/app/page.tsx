...
function parseDayOffset(raw: string | undefined): number {
  ...
  return Math.min(Math.max(value, -30), 0)
}

function getDisplayDate(timezone: string, dayOffset: number): Date {
  const nowInTz = new Date(
    new Date().toLocaleString('en-US', { timeZone: timezone }),
  )
  nowInTz.setDate(nowInTz.getDate() + dayOffset)
  return nowInTz
}

const navButtonBase =
  'w-8 h-8 flex items-center justify-center rounded-xl border-2 border-blue-deep bg-white dark:bg-dark-card text-blue-deep shadow-[3px_3px_0_#0D4F78] transition-opacity'
...
export default async function DashboardPage({
  ...
  const safeOffset = parseDayOffset(day)
  const timezone = profile.timezone ?? 'UTC'
  const goal = profile.daily_goal_ml ?? 2500
  const isToday = safeOffset === 0
  const displayDate = getDisplayDate(timezone, safeOffset)
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className={`${navButtonBase} hover:opacity-70`}
            >
            ...
            {isToday ? (
              <span
                aria-disabled="true"
                aria-label="Next day"
                className={`${navButtonBase} opacity-30 cursor-not-allowed`}
              >
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
